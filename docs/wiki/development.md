...
- `src/workspace/` — local coding boundary: `mod.rs` owns bounded file/search/edit/move/delete primitives; `roots.rs` and `registry.rs` enforce root/workspace isolation; `fs_safety.rs` contains filesystem safety helpers; `command_policy.rs` owns no-shell command policy; `authorization.rs` handles local human approval; `conventions.rs` reports repository conventions/architecture; `scheduler.rs` builds path-resource dependencies and safe same-file/same-SHA edit coalescing.
...
Pages workflow builds Markdown under `docs/wiki/` into HTML below `/wiki/`
through Jekyll, then uploads one `_site` artifact. All maintained product,
integration, architecture, development, and release Markdown belongs under
`docs/wiki/` and must be reachable from `docs/wiki/README.md`; root Markdown is
