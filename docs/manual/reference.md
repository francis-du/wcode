...
| `semantic_navigation` | Reuse the warm LSP session for symbol-first definition/hover, references, incoming/outgoing calls, implementations, cross-file impact, or `rename_plan`. Read-only navigation may explicitly fall back to Tree-sitter/search; rename never does. `rename_plan` requires `symbol + new_name`, returns a complete SHA-pinned `apply_file_edits` plan, rejects external/resource/stale/partial edits, and never writes automatically. |
