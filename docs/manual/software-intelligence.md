...
Use `semantic_navigation` for relationship questions where repository text search is incomplete: definitions/hover when semantic resolution matters, references, implementations, incoming callers, outgoing callees, a bounded impact bundle, or a guarded semantic rename plan. Prefer the `path + symbol` form; wcode resolves the symbol through its syntax index and converts its 1-based UTF-8 byte position into the position encoding negotiated with the LSP server, so the agent does not calculate UTF-16 offsets. A direct `line + character` selector remains available for callers that already own a precise source position. For rename, call `semantic_navigation` with `intent=rename_plan` plus `new_name`; wcode asks the live LSP for the complete WorkspaceEdit, rejects external/resource/partial/stale edits, converts accepted ranges into SHA-pinned `apply_file_edits` arguments, and never applies them automatically. Rename has no syntax fallback: if semantic completeness is unavailable, the mutation fails closed instead of guessing cross-file targets.
