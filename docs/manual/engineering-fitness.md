...
| Non-Gold Symbol Fraction | Returned identities outside that task's authored required/useful set. `required` means evidence that must be delivered to satisfy the task; `useful` means adjacent evidence that is relevant but not mandatory, such as the direct counterpart in a caller/callee pair or a behavior-specific regression test. Everything else remains measurable Non-Gold; generic module/re-export filler is not promoted merely to improve the score. This is not a claim about irrelevant tokens across the entire response. |
...
Contract v3 validates each response against the requested experiment budget, not just the tool's self-reported limit. Failed answerable attempts contribute zero to average NDCG; `ranking_attempts` and `noise_response_samples` expose their different denominators. Undefined human-readable percentages are `N/A`, matching JSON `null`. Case membership and required Gold remain frozen; useful annotations are separately authored and regression-tested so direct task relationships and behavior-specific tests are not misclassified as noise, while unrelated wrappers/re-exports remain Non-Gold. The fraction is therefore annotation-relative and still is not an irrelevant-token rate. JSON rows retain exact `non_gold_identities`, and the Markdown per-case table exposes both NDCG@10 and Non-Gold for diagnosis. The JSON container schema remains version 2; the measurement contract and evaluator fingerprint identify the changed scoring protocol. Regrade older reports before numerical comparison when evaluator annotations or failure accounting differ.
