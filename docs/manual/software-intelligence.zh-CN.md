...
任务活动与项目快照独立刷新。有任务或待批准请求时，活动请求完成后间隔 2 秒再刷新；空闲或数据不可用时为 8 秒。项目版本检查使用独立的 8 秒调度。这是轮询间隔，不是延迟保证；首次活动查询也不必等待架构重建。每条读取流程只允许一个在途请求，页面隐藏时停止轮询，权限修改超时后不会自动重放。

全部授权写操作同时绑定工作区和选择代次，连 A → B → A 的迟到响应也会拒绝。统一的在途操作管理避免连续回车、双击造成重复提交；旧读取不能覆盖已完成的批准或新项目的权限面板。授权列表获取失败、内存尚未采样时显示未知，不显示为零。

`proof.current_*` 保留当前版本历史记录计数；`proof.effective` 另外按目标、证据类型、来源、模型、置信度、目标集合和策略分别选择最新结果。更晚的已知完整检查可替代同范围的快速检查，不能反过来；不同来源、不同检查范围仍独立。人工批准不算测试证据。详情优先列出失败，最多返回 24 行，摘要有界且脱敏，截断明确标记。这只是观测视图，不是发布门禁，也不会删除审计历史。

验证历史在一次请求内共用一份代码／设计版本和一份证据输入，空历史不做这两次读取。观测页复用自己的输入计算计划状态；下一次请求重新读取，确保代码修改和新增证据可见。轻量证据变更信号只处理有界记录身份和元信息，能发现迟到记录及删除，不再解析全部证据正文；该信号不充当验证证据。持久化记录保留上限仍然生效。

### 30 轮检查对应关系

轮次表示不同场景，不是重复运行 30 次同一套全量测试，也不是 30 项独立新功能。第 1–20 轮由 `tests/unit/ui/audit.cjs` 分别输出结果，并通过 Rust 单元测试入口执行。报告位于 `target/wcode-audit.json`，验证历史的真实读取次数对照位于 `target/wcode-history-perf.json`。

| 轮次 | 检查范围与断言 |
| --- | --- |
| 1–4 | 跨项目的程序授权／撤销响应；重复授权／撤销提交。 |
| 5–8 | 迟到的项目创建、重复添加项目、旧读覆盖新写、A → B → A 响应。 |
| 9–12 | 批准数量顺序、授权读取失败、内存未采样、活动不可用。 |
| 13–16 | 严重性排序、HTML 格式 HTTP 错误、项目刷新阻塞时的活动更新、隐藏／暂停调度。 |
| 17–20 | 首屏并发活动查询、单轮询循环、恢复后的有效证据、详情转义与截断。 |
| 21 | 独立计算与共享输入的验证历史对照：实际读取次数与返回结果完全等价。 |
| 22 | 外工作区计划在读取输入前拒绝；空历史不扫描；下一请求识别修改与新证据。 |
| 23 | 重试范围、时间戳冲突、目标归一化、完整／快速单向替代、批准与测试证据区分。 |
| 24 | 验收冲突、代码加设计双版本新鲜度、有界脱敏详情与历史保留。 |
| 25 | 元信息变更信号识别迟到／删除记录，拒绝符号链接记录。 |
| 26 | 原有界面行为、语法、响应式布局契约、接口保护与双语文档兼容性。 |
| 27 | 工作树差异、Rust 格式、类型检查。 |
| 28 | 覆盖测试源码的全目标 Clippy，告警视为失败。 |
| 29 | 完整 Rust 测试集，包含 JavaScript 行为测试及集成／契约测试。 |
| 30 | 优化构建与验证记录／版本一致性核对。 |

这些自动化检查不能代替真实浏览器视觉／无障碍验收、远程连接测试或跨平台 CI；安装、重新连接当前进程仍是独立的操作步骤。
...
`current_*` 证据计数只统计与当前代码和设计版本一致的 Evidence；验收映射、运行、通过和新鲜度仍分别展示。本地 Agent 在项目工作目录中使用
...
`semantic_navigation` 既处理纯文本搜索不完整的 Relationship，也负责受保护的 LSP Mutation Plan。Symbol 关系与 Rename 优先传 `path + symbol`；wcode 用 Tree-sitter 定位 Symbol，再把 1-based UTF-8 Byte Position 转成 LSP Server 协商出的 Position Encoding。`intent=rename_plan` 配合 `new_name` 请求完整 WorkspaceEdit，并返回带当前 SHA 的 `apply_file_edits` 参数；`intent=organize_imports_plan` 只接受当前文件唯一的 Edit-only Action；`intent=quick_fix_plan` 必须命中 `line + character` 上当前 Document Version 的有界 `publishDiagnostics`，多个安全 Fix 没有唯一 Preferred 时也会拒绝猜测。Provider Command 永远不会执行；Resource Operation、外部或多文件 Code Action、过期/部分计划、敏感 Guarded Source 与歧义 Edit 都 Fail Closed。所有 Semantic Mutation Intent 都没有 Syntax Fallback，也绝不会自动落盘。

`intent` 决定实际发哪些 LSP Request：`definition`、`hover`、`references`、`incoming_calls`、`outgoing_calls`、`calls`、`implementations`、`impact`、`rename_plan`、`organize_imports_plan`、`quick_fix_plan`。其中 `impact` 偏向跨文件完整性，只查询 Reference、Incoming Caller 和 Implementation，而不是把所有 LSP 能力都扫一遍。Result 会把 `unsupported` 与 `failures` 分开：空 Relationship List 只表示“这个能力受支持、请求成功、没有匹配关系”；LSP Timeout/Error 会单独暴露，绝不会被当成 Negative Semantic Evidence。没有可信 LSP Server 时，Tool 明确返回 `precision=syntax` 与 `routing=tree_sitter_fallback`，不会伪装成 Semantic Precision。普通“这个 Symbol 在哪”仍然使用 `find_symbol` / `search_code`，让 LSP 成本只花在真正需要 Semantic Completeness 的任务上。
