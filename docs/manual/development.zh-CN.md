...
托管隧道与本地 HTTP 服务独立运行。`auto` 并发启动 Cloudflare、localhost.run、Pinggy 与 Tailscale Funnel；所有验证成功的入口同时可用，不再区分主备。每个 URL 独立维护身份绑定的健康状态、探测任务与恢复流程，健康时每三秒探测，异常时每秒重试，250 毫秒调度周期不等待网络请求。探测使用轻量 `/healthz/probe`，冷连接允许十秒、请求总计十五秒，必须验证 `ok=true` 和当前 `instance_id`。连续两次失败进入健康隔离，保留原进程、URL 与 Host 信任三十秒以便原地恢复；连续两次成功才恢复已验证状态。界面用于复制的默认 URL 不享有路由或健康优先权。稳定入口在子进程退出后只保留原有有界健康记录，由独立任务继续检查，不从历史 OAuth 资源恢复信任；所有清理均检查注册 Epoch。启动取消和退出只清理自身进程组，不扫描删除操作者进程；Auto 不提前截断 Provider 的启动期限。Cloudflare 返回 HTTP 429 / error 1015 时冷却五分钟，其他故障按有界退避独立重试。首次启动失败也会显示诊断与重试状态，未验证 URL 不会发布。TUI、Setup 与 Observatory 同时展示所有入口，OAuth、Origin、Workspace 和实例身份校验保持完整。

入口信任与进程健康分别管理。保留的稳定地址按 URL 索引，最多八个；即使同一 Provider 换地址连接，旧地址仍由独立探测监督。重连启动失败只安排重试，不自行撤销保留地址的信任。恢复入口统一进入 Connected 生命周期，不由独立发布任务旁路注册。每次验证注册获得新的代次，清理时在同一锁内比对记录的代次再撤销；展示顺序变化不改变所有权代次。旧清理不能撤销新连接的 Host，也不能用重试行覆盖其遥测。维护采用持续 interval，避免不断到来的事件反复推迟维护时点。
...
- Git Mutation 只允许 Explicit-path `git add`、Message-only `git commit`、Explicit Remote+Ref Non-force `git push` 进入授权。远端同步必须获得精确 `RiskyExecution`，且只接受 `git ls-remote <已配置名称> <明确 heads/tags ref>` 或 `git fetch <已配置名称> <单一明确分支>`；任意 URL、选项、通配／强制 Refspec、Prune 与 Pull 继续不可用，Fetch 按 Workspace Write 管理。批准后的 SSH Push 只允许通过 wcode 固定 Non-interactive SSH Command 使用当前 `SSH_AUTH_SOCK`；Token-like Env、Credential Helper、AskPass、任意 Git Config、Proxy Helper、HTTP Extra Header、Hook、External Diff 仍被移除/禁用；
