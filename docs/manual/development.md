...
- `src/main.rs` — CLI and startup composition: argument parsing, runtime lifecycle, graceful shutdown, and Product Scope module wiring. Managed tunnel lifecycle is delegated to `src/runtime/tunnel.rs`.
...
- `src/runtime/` — Harness/runtime orchestration. `harness.rs` owns the public core; `harness_agent_context.rs` builds edit-ready Agent Context; `harness_profile.rs` caches project/manifests/guidance; `harness_repo_map.rs` ranks/caches repo-map structure; `harness_graph.rs`, `harness_project.rs`, `harness_review.rs`, `harness_scope.rs`, `harness_text.rs`, and `harness_verification.rs` split graph/context, Project Observatory assembly, Git review, Product Scope audit, text/project discovery, and verification. `semantic.rs` owns automatic bounded semantic freshness across the most-specific Workspaces. `harness_tests.rs` contains focused Harness tests. `power.rs` and `tunnel.rs` own idle-sleep inhibition and managed public tunnels. Foreground shutdown is OS-signal driven; stdio lifecycle belongs to the MCP Host.
- `src/integrations/` — model/client integration boundary. `mcp.rs` owns shared HTTP routing and payload dispatch; `mcp_stdio.rs` and `mcp_legacy_sse.rs` adapt stdio and legacy SSE to that dispatch; `mcp_dispatch.rs`, `mcp_tools.rs`, `mcp_tasks.rs`, and `mcp_catalog.rs` cover Tool calls, the catalog, durable Tasks, Prompts, and Resources. `auth.rs` owns OAuth/PKCE/DCR while `auth_origin.rs` resolves the exact registered request origin. `agent_plugin.rs` exports canonical package files; the `agent_install*` modules separate Host adapters, safe merge/apply, reporting, and tests.
...
- `src/ui/` — operator experience. `monitor.rs` owns the Ratatui event/render loop, `monitor_state.rs` owns task/connection/traffic metrics, `monitor_detail.rs` owns panels and overlays, and `monitor_theme.rs` keeps the TUI palette aligned with `docs/assets/site.css`. `intelligence_web.rs` serves the protected Project Observatory.
...
Managed public tunnels are owned runtime children, separate from the local HTTP server. `--tunnel-provider auto` starts Cloudflare, `localhost.run`, Pinggy, and Tailscale Funnel concurrently in the background; the dashboard never waits for them. A tunnel counts as live only after URL discovery and instance-matched `/healthz` succeeds — a URL string alone is not readiness. Every live tunnel is kept; the first to land owns the primary endpoint. A dead tunnel respawns alone after bounded exponential backoff starting at 5 seconds and capped at 300 seconds; a verified reconnect clears that provider's death history so later incidents start fast again. If the primary dies the next live tunnel is promoted; if every tunnel disappears, public-health state returns to pending and the local endpoint remains available while recovery runs. Quick-tunnel providers may return a new hostname after reconnect, so durable remote clients need a stable endpoint such as Tailscale Funnel or an operator-managed `--public-url`. Normal shutdown (Ctrl-C or SIGTERM) aborts owned tasks and kills/waits for every tunnel child. Never implement recovery by killing or replacing unrelated operator processes.
...
- every indexed language must keep exactly one contract-tested canonical LSP launch profile; provider-specific executable aliases/arguments (including Workspace/runtime-unique state such as JDT LS `-data`) belong in the semantic provider adapter, and only proven alternates may remain in fallback order; compatibility tests do not pretend the external server is installed, while live initialize failures must remain observable and may fall through only to a separately trusted installed alternate;
- automatic first-party LSP execution is limited to built-in providers with an explicit hardened profile: the executable must resolve outside the Workspace, stdio/output/time stay bounded, credential and execution-injection environment variables are scrubbed, and provider-specific repository-code execution is disabled where supported; warm sessions are capacity/idle bounded, keyed to Workspace + provider-binary identity, synchronize documents by source revision, and never bypass result-side Workspace filtering; `--no-semantic` disables the lane; non-profiled LSPs and advanced verification executors still require explicit trust unless process-wide `--allow-risky-exec` intentionally pre-authorizes them;
