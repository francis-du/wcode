...
The global semaphore remains the total tool-admission cap, not a promise that every admitted tool can execute a child process at once. Process-executing MCP tools and project checks first acquire a separate execution-admission permit sized to the smaller of the outer headroom policy and heavy-process capacity. With the balanced 32-tool / 4-process profile this is 4 heavy-command admissions, so excess commands wait before taking a global Tool permit instead of allowing 28 commands to pile up behind four child-process permits. Process admission wait is capped at five seconds independently from the command execution timeout. Both permits follow real work, including a started blocking worker after caller cancellation. Inner CPU, I/O and process queues retain their own bounded capacities. Composite operations must not retain a parent permit while children wait for permits. `parallel_tools`, `review_changes`, `verify_project`, and other internally fanned-out operations therefore schedule real child work through the same global accounting path.
...
Keyboard input resolves once through `dashboard_action` before side effects. Input/confirmation contexts are exclusive, modifier chords never become unmodified grants, and permission/toggle/link actions require Press rather than Repeat. `A` only authorizes the visible request's Workspace; `B` is the author link. Keep help labels and the context/modifier regression matrix in sync.

Observatory content grids must use content-driven height and shrinkable tracks. Spacing classes must not fix the height of content containers; nested code statistics and verification previews wrap unbounded identifiers. Run `node tests/unit/ui/layout.cjs .`; on macOS, follow with `swift tests/unit/ui/layout_webkit.swift` for real-engine bounds/overlap checks at mobile, tablet and desktop widths. The fixture contains production-renderer HTML, shipped CSS and synthetic data, with no live credentials or network access.

Managed public tunnels are owned runtime children, separate from the local HTTP server. `auto` starts Cloudflare, localhost.run, Pinggy and Tailscale Funnel concurrently. All instance-verified tunnel URLs are concurrent active MCP endpoints, never primary or standby. Each endpoint has its own identity-bound single-flight health lease and independent recovery worker. Healthy probes run every three seconds and suspect/recovery probes every second; the scheduler ticks every 250 ms without waiting on network requests. Probes use the compact /healthz/probe response, allow ten seconds for a cold connection and fifteen seconds total, and require ok=true plus the current instance_id. Two failed probes quarantine health; the existing URL and Host trust survive a bounded 30-second in-place recovery window and two consecutive successes restore verified health. A convenience URL for local copying does not grant exclusive routing or health authority. Previously verified stable aliases preserve only their existing bounded lease across child exit and continue independent probes; historical OAuth resources never restore Host trust. Registration epochs protect same-URL replacements from stale cleanup. Provider subprocess groups are owned from spawn, including startup cancellation and shutdown, with no global orphan scan. Provider-specific startup deadlines are not truncated by auto mode. Explicit Cloudflare HTTP 429 / error 1015 receives a five-minute cooldown in both startup and reconnect; ordinary failures retain bounded staggered retries. Initial failures remain observable without publishing unverified links. Every active URL is usable in Setup, TUI and Observatory. OAuth, Origin and Workspace authorization remain enforced on every endpoint.

Endpoint trust and process health have distinct ownership. Retained stable aliases are keyed by URL, bounded to eight and independently probed even after a provider connects at a different address. Reconnect-startup failures schedule retries but never revoke retained trust themselves. All recovered endpoints enter the normal Connected lifecycle, not a detached publisher. Each verified registration receives a new epoch; cleanup compares its recorded epoch atomically before withdrawing the alias, and display-order changes never renew ownership. Stale cleanup cannot remove a replacement's Host or overwrite its telemetry with a retry row. The maintenance timer is a persistent interval so incoming events cannot indefinitely reset its deadline.
...
- Git mutation only through explicit-path `git add`, message-only `git commit`, and explicit remote+ref non-force `git push`. Remote synchronization requires exact `RiskyExecution` and accepts only `git ls-remote <configured-name> <explicit heads/tags ref>` or `git fetch <configured-name> <single explicit branch>`; arbitrary URLs, options, wildcard/force refspecs, prune and pull remain unavailable, and fetch is classified as a Workspace write. An approved SSH push may use the current `SSH_AUTH_SOCK` only through wcode's fixed non-interactive SSH command. Token-like environment variables, Credential Helper, AskPass, arbitrary Git config, proxy helpers, HTTP extra headers, hooks, and external diff helpers remain stripped/disabled;
