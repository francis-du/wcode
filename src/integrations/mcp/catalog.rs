...
const IMPLEMENT_PROMPT: &str = "Use wcode as the repository control layer. Call agent_context first with the coding goal and known Product Scope(s); omit budget for bounded adaptive sizing. Follow readiness/next_actions and do not reread hot_source. Use symbol_context only for missing bodies, apply_edits for one direct target file or apply_file_edits for multiple, then review_changes and the recommended verify_project level. Reuse existing components before adding abstractions. Use deeper design, drift, risk, reconciliation, or language-quality tools only when required. Never promote syntax facts to semantic precision or fabricate verification evidence.";
...
const SECURITY_RESOURCE: &str = "wcode is workspace-scoped. Absolute paths, parent traversal, protected credential/VCS paths, symlink aliases, unsafe hard-link writes, stale SHA writes, unrestricted shell execution, and model-facing delete are denied. Hardened first-party semantic providers may run automatically through the bounded LSP lane; repository-local LSP executables are rejected and --no-semantic disables the lane. Other repository-aware language servers and custom stage executors still require explicit RiskyExecution trust. Agent plugins should prefer stdio MCP for local agents and must not bundle credentials, executable hooks, or hidden shell scripts by default.";
