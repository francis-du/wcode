...
fn build_tools() -> Vec<Value> {
    vec![
        ...
        tool("semantic_provider_status", "Report detected languages, selected LSP providers, readiness/action, install plan, and syntax fallback.", schema(json!({}), &[]), true, false),
        tool("semantic_provider_install", "Install one canonical missing LSP through a fixed wcode provider plan. Model-installable plans require exact RiskyExecution approval; manual/toolchain-specific providers return guidance instead of inventing commands.", schema(json!({"language":{"type":"string","enum":["bash","c","cpp","c-sharp","css","dart","elixir","go","html","java","java-script","lua","ocaml","ocaml-interface","php","python","r","ruby","rust","swift","type-script","tsx"]}}), &["language"]), false, false),
        ...
        tool("graph_diff", "Compare persisted graph revisions by stable node/edge identity; report bounded real additions, removals and changed provenance. Omit IDs for latest two.", schema(json!({"diff":{"type":"object","properties":{"from_snapshot_id":{"type":"string","minLength":1,"maxLength":160},"to_snapshot_id":{"type":"string","minLength":1,"maxLength":160},"limit":{"type":"integer","minimum":1,"maximum":200,"default":50}},"additionalProperties":false}}), &[]), true, false),
