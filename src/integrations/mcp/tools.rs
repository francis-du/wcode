...
fn build_tools() -> Vec<Value> {
    vec![
        ...
        tool("run_command", "Run a policy-checked program without a shell. Bounded repository-local verification commands (test/check/lint/analyze/build/format-check) run autonomously; only commands with broader side effects require human authorization. Long-running builds may request up to 1800 seconds; timeouts return failed results with captured output so effects can be inspected before retrying.", schema(json!({"program":{"type":"string","minLength":1,"maxLength":256,"description":"Bare executable name. Bounded verification tools are pre-authorized; non-default or side-effectful programs require explicit per-workspace human authorization."},"args":{"type":"array","items":{"type":"string"}},"cwd":{"type":"string"},"timeout_seconds":{"type":"integer","minimum":1,"maximum":1800}}), &["program"]), false, true),
    ]
}
