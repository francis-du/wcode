...
fn build_tools() -> Vec<Value> {
    vec![
        ...
        tool("semantic_provider_install", "Install one canonical missing LSP via wcode's fixed provider plan and required approval.", schema(json!({"language":{"type":"string","enum":["bash","c","cpp","c-sharp","css","dart","elixir","go","html","java","java-script","lua","ocaml","ocaml-interface","php","python","r","ruby","rust","swift","type-script","tsx"]}}), &["language"]), false, false),
        ...
        tool("semantic_provider_refresh", "Refresh read-only LSP symbols/relationships under provider trust policy.", schema(json!({"path":{"type":"string","default":"."},"max_files":{"type":"integer","minimum":1,"maximum":256,"default":128},"max_symbols":{"type":"integer","minimum":1,"maximum":2000,"default":1000}}), &[]), false, false),
        tool("semantic_navigation", "Use warm read-only LSP for bounded cross-file navigation.", schema(json!({"path":{"type":"string"},"symbol":{"type":"string","minLength":1,"maxLength":300},"line":{"type":"integer","minimum":1},"character":{"type":"integer","minimum":1},"intent":{"type":"string","enum":["inspect","definition","hover","references","incoming_calls","outgoing_calls","calls","implementations","impact"],"default":"inspect"},"max_results":{"type":"integer","minimum":1,"maximum":100,"default":50}}), &["path"]), true, false),
