...
fn build_tools() -> Vec<Value> {
    vec![
        ...
        tool("semantic_provider_status", "Report detected languages, selected LSP providers, readiness/action, and syntax fallback.", schema(json!({}), &[]), true, false),
        tool("language_quality_status", "Report per-language syntax/semantic/quality/verification capabilities and explicit gaps.", schema(json!({}), &[]), true, false),
        tool("language_quality_run", "Run one declared check-only language quality provider and persist current-revision Evidence.", schema(json!({"language":{"type":"string","enum":["bash","c","cpp","c-sharp","css","dart","elixir","go","html","java","java-script","lua","ocaml","ocaml-interface","php","python","r","ruby","rust","swift","type-script","tsx"]},"provider_id":{"type":"string","minLength":1,"maxLength":160},"timeout_seconds":{"type":"integer","minimum":1,"maximum":1800,"default":120}}), &["language","provider_id"]), false, false),
        tool("semantic_provider_refresh", "Refresh read-only LSP symbols/relationships. Hardened rust-analyzer/gopls are automatic; other providers require exact trust.", schema(json!({"path":{"type":"string","default":"."},"max_files":{"type":"integer","minimum":1,"maximum":256,"default":128},"max_symbols":{"type":"integer","minimum":1,"maximum":2000,"default":1000}}), &[]), false, false),
        tool("semantic_navigation", "Use warm read-only LSP for cross-file relationships; on unavailable LSP, return explicit syntax/search degradation.", schema(json!({"path":{"type":"string"},"symbol":{"type":"string","minLength":1,"maxLength":300},"line":{"type":"integer","minimum":1},"character":{"type":"integer","minimum":1},"intent":{"type":"string","enum":["inspect","definition","hover","references","incoming_calls","outgoing_calls","calls","implementations","impact"],"default":"inspect"},"max_results":{"type":"integer","minimum":1,"maximum":100,"default":50}}), &["path"]), true, false),
        ...
        tool("verification_stage_submit", "Submit real Property, Mutation, Fuzz, or Runtime/Canary stage evidence for a Verification Plan. New target-scoped plans require the producer to name the plan targets covered by this artifact; readiness aggregates fail closed by both producer and target, so one language or subsystem cannot clear another target's gate. Legacy targetless plans remain readable.", schema(json!({"plan_id":{"type":"string","minLength":1,"maxLength":160},"submission":{"type":"object","properties":{"stage":{"type":"string","enum":["property","mutation","fuzz","runtime_canary"]},"producer":{"type":"string","minLength":1,"maxLength":256},"verdict":{"type":"string","enum":["pass","fail","inconclusive"]},"summary":{"type":"string","minLength":1,"maxLength":2000},"artifact_digest":{"type":"string","minLength":1,"maxLength":512},"targets":{"type":"array","maxItems":32,"items":{"type":"string","minLength":1,"maxLength":128}},"model":{"type":"string","minLength":1,"maxLength":256}},"required":["stage","producer","verdict","summary","artifact_digest"],"additionalProperties":false}}), &["plan_id","submission"]), false, false),
        ...
        tool(
            "parallel_tools",
            "Run compact dependency-aware batches; dry_run previews dependencies without executing. Children inherit Workspace; ready successors start immediately and failed dependencies skip.",
            schema(json!({
        ...
        tool("run_command", "Run a command under the Workspace execution policy. Normal mode keeps bounded repository-development safeguards; an explicit Workspace all-command grant or Full Access disables command-policy filtering while resource limits, timeout, cancellation, bounded output, and Workspace cwd selection remain enforced. Long-running commands may request up to 1800 seconds.", schema(json!({"program":{"type":"string","minLength":1,"description":"Executable name or path. Normal mode applies Workspace command policy; explicit all-command authorization or Full Access permits unrestricted command shapes."},"args":{"type":"array","items":{"type":"string"}},"cwd":{"type":"string"},"timeout_seconds":{"type":"integer","minimum":1,"maximum":1800}}), &["program"]), false, true),
    ]
}
