...
pub(super) fn tools() -> Vec<Value> {
    vec![
        ...
        tool("semantic_provider_status", "Auto-detect source languages and first-party LSP semantic providers for every language supported by wcode's syntax index. Every indexed language has one canonical launch profile; status reports selected provider, canonical/alternate state, available candidate count, policy readiness, and honest syntax fallback when no server is available.", schema(json!({}), &[]), true, false),
        ...
        tool("semantic_provider_refresh", "Force a bounded first-party LSP semantic refresh and persist real Document Symbol / Call Hierarchy facts into the Software Graph. If a canonical non-automatic provider fails initialization, an installed alternate may be tried and is reported in fallbacks. Hardened automatic providers are maintained in the background by default; providers without an automatic safety profile still require explicit RiskyExecution trust. --no-semantic disables all first-party LSP execution.", schema(json!({"path":{"type":"string","default":"."},"max_files":{"type":"integer","minimum":1,"maximum":256,"default":128},"max_symbols":{"type":"integer","minimum":1,"maximum":2000,"default":1000}}), &[]), false, false),
        tool("semantic_navigation", "Use the warm LSP session for cross-file references, calls, implementations, impact, or hover. Prefer find_symbol/search_code for simple localization; symbol-first selectors avoid model-computed LSP offsets, syntax fallback stays explicit, and LSP failures are reported separately from unsupported or successfully empty relationships.", schema(json!({"path":{"type":"string"},"symbol":{"type":"string","minLength":1,"maxLength":300},"line":{"type":"integer","minimum":1},"character":{"type":"integer","minimum":1},"intent":{"type":"string","enum":["inspect","definition","hover","references","incoming_calls","outgoing_calls","calls","implementations","impact"],"default":"inspect"},"max_results":{"type":"integer","minimum":1,"maximum":100,"default":50}}), &["path"]), true, false),
