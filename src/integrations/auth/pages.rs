...
<style>
:root{{--bg:#0b0812;--panel:#151020;--raised:#1c1529;--line:#2f2739;--line2:#493d58;--text:#faf7ff;--muted:#a89db7;--soft:#cec5da;--accent:#8b7cff;--danger:#c27788;--fontSans:"Inter","Noto Sans SC","Noto Sans",sans-serif;--fontMono:"JetBrains Mono","Noto Sans Mono",monospace}}
...
label{{display:block;font-size:13px;font-weight:650;color:var(--soft);margin:0 0 7px}}input.code{{width:100%;height:46px;border-radius:12px;border:1px solid var(--line2);background:#120d1c;color:var(--text);outline:none;padding:0 12px;font:600 19px/1 var(--fontMono);letter-spacing:7px;text-align:center}}input.code:focus{{border-color:var(--accent);box-shadow:0 0 0 2px #8b7cff20}}input.code::placeholder{{font:400 12px var(--fontSans);letter-spacing:0;color:#746982}}
...
.foot{{display:flex;justify-content:space-between;align-items:center;gap:7px 12px;flex-wrap:wrap;margin-top:10px;font-size:12px;color:var(--muted)}}a{{display:inline-flex;align-items:center;min-height:34px;color:var(--soft);text-decoration:none}}a:hover{{color:var(--text)}}.dot{{width:6px;height:6px;border-radius:50%;background:#879583;display:inline-block;margin-right:6px}}@media(max-width:520px){{body{{place-items:start center}}.card{{padding:16px}}h1{{font-size:20px}}.scope{{margin:13px 0;padding:9px}}input.code{{height:48px;padding:0 9px;font-size:19px;letter-spacing:6px}}button,a{{min-height:44px}}.foot{{align-items:flex-start;justify-content:flex-start}}}}
</style>
</head>
