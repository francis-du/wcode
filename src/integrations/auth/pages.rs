...
pub(super) fn simple_auth_html(title: &str, message: &str, endpoint: Option<&str>) -> String {
    let endpoint_html = endpoint
        ...
        .unwrap_or_default();
    format!(
        r##"<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="dark"><meta name="theme-color" content="#09090b"><title>{title} · wcode</title><style>*{{box-sizing:border-box}}body{{margin:0;min-height:100vh;min-height:100svh;min-height:100dvh;display:grid;place-items:center;background:#09090b;color:#f5f5f6;font:15px/1.55 ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;padding:max(16px,env(safe-area-inset-top)) max(16px,env(safe-area-inset-right)) max(16px,env(safe-area-inset-bottom)) max(16px,env(safe-area-inset-left));overflow-x:hidden;-webkit-text-size-adjust:100%;text-size-adjust:100%}}main{{width:min(100%,520px)}}.card{{padding:28px;border:1px solid #29292f;border-radius:18px;background:#111114;box-shadow:0 28px 80px #0008}}h1{{margin:0 0 8px;font-size:24px;line-height:1.2}}p{{margin:0;color:#a1a1aa;overflow-wrap:anywhere}}.endpoint{{margin-top:20px;padding:13px 15px;border:1px solid #323239;border-radius:12px;background:#09090b;color:#fafafa;font:12px ui-monospace,SFMono-Regular,Menlo,monospace;overflow:auto;overflow-wrap:anywhere;word-break:break-word;user-select:all;-webkit-user-select:all;-webkit-overflow-scrolling:touch}}.links{{display:flex;gap:8px 16px;flex-wrap:wrap;margin-top:18px}}a{{display:inline-flex;align-items:center;min-height:40px;color:#fff}}@media(max-width:520px){{body{{place-items:start center}}.card{{padding:20px;border-radius:14px}}h1{{font-size:21px}}}}</style></head><body><main><section class="card"><h1>{title}</h1><p>{message}</p>{endpoint_html}<div class="links"><a href="{project_url}" target="_blank" rel="noreferrer">Project ↗</a><a href="{author_url}" target="_blank" rel="noreferrer">{author_handle} ↗</a></div></section></main></body></html>"##,
...
*{{box-sizing:border-box}}body{{margin:0;min-height:100vh;min-height:100svh;min-height:100dvh;display:grid;place-items:center;background:radial-gradient(900px 500px at 50% -15%,#25252d 0%,var(--bg) 62%);color:var(--text);font:15px/1.55 ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;padding:max(16px,env(safe-area-inset-top)) max(16px,env(safe-area-inset-right)) max(16px,env(safe-area-inset-bottom)) max(16px,env(safe-area-inset-left));overflow-x:hidden;-webkit-text-size-adjust:100%;text-size-adjust:100%}}
...
.foot{{display:flex;justify-content:space-between;align-items:center;gap:8px 14px;flex-wrap:wrap;margin-top:16px;padding:0 4px;font-size:12px;color:#73737d}}a{{display:inline-flex;align-items:center;min-height:36px;color:#b8b8c0;text-decoration:none}}a:hover{{color:#fff}}.dot{{width:7px;height:7px;border-radius:50%;background:#5ee28a;box-shadow:0 0 12px #5ee28a99;display:inline-block;margin-right:7px}}@media(max-width:520px){{body{{place-items:start center}}.brand{{margin-bottom:12px}}.card{{padding:20px;border-radius:14px}}h1{{font-size:21px}}.scope{{margin:17px 0;padding:11px}}input.code{{height:52px;padding:0 10px;font-size:21px;letter-spacing:6px}}.foot{{align-items:flex-start;justify-content:flex-start}}}}
</style>
</head>
