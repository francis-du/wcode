...
function renderProofSummary() {
  ...
  const selectedEntry = keyed.find(entry => entry.key === state.selectedEvidenceKey) || keyed[0], selected = selectedEntry?.item,
    ...
    metricCards = [
      ...
      [localized("Passing evidence", "通过证据"), num(effectiveProof().current_passed || 0), localized(`${num(effective.total || rows.length)} effective records`, `${num(effective.total || rows.length)} 条有效记录`), "good", "check"],
...
function renderActivity() {
  ...
  const agent = activity.agent_context || {}, agentCalls = Number(agent.calls || 0),
    ...
    cacheHits = Number(agent.repo_map_cache_hits || 0), avgBuild = agentCalls > 0 ? Number(agent.build_ms || 0) / agentCalls : null,
    modelTokens = Number(agent.model_tokens || 0), budgetTokens = Number(agent.budget_tokens || 0),
  ...
  const byCheckpoint = jev?.calls?.by_checkpoint || {};
  const checkpoint = String(jev?.checkpoint || "agent_context").replaceAll("_", " ");
  const checkpointMix = Object.entries(byCheckpoint).filter(([,count])=>Number(count)>0).map(([name,count])=>`${name.replaceAll("_"," ")} ${num(count)}`).join(" · ") || "—";
  ...
  const latestCall = jev?.call || {}, latestTokens = latestCall.tokens || {}, callTotals = jev?.calls || {}, totalTokens = callTotals.tokens || {};
  const tokenSourceLabel = source => ({
    provider_reported: localized("provider usage", "Provider 精确 usage"),
    byte_estimate: localized("byte estimate", "按字节估算"),
    mixed_provider_and_byte_estimate: localized("mixed provider + estimate", "Provider + 字节估算混合"),
    unavailable: localized("unavailable", "不可用"),
  })[source] || source || "—";
  const latestTokenSummary = `${num(latestTokens.input || 0)} in / ${num(latestTokens.output || 0)} out`;
  const totalTokenSummary = `${num(totalTokens.input || 0)} in / ${num(totalTokens.output || 0)} out`;
  const latestIo = `${formatBytes(Number(latestCall.request_bytes || 0))} → ${formatBytes(Number(latestCall.response_bytes || 0))}`;
  const totalIo = `${formatBytes(Number(callTotals.request_bytes || 0))} → ${formatBytes(Number(callTotals.response_bytes || 0))}`;
  const jevRuntime = decision ? `<section class="agent-context-efficiency"><header class="resource-telemetry-head"><strong>${esc(localized("Jev decision runtime", "Jev 决策运行态"))}</strong><span>${esc(localized("Latest real decision-checkpoint outcome · bounded process-memory telemetry", "最近一次真实决策检查点结果 · 有界进程内遥测"))}</span></header>${jev ? `<div class="proof-counts resource-counts"><div><strong>${esc(jevStatusText)}</strong><span>${esc(localized(`Jev · ${successful}/${observed} successful · ${duration(Number(jev.observed_ago_ms || 0))} ago`, `Jev · ${successful}/${observed} 成功 · ${duration(Number(jev.observed_ago_ms || 0))} 前`))}</span></div><div><strong>${esc(checkpoint)}</strong><span>${esc(localized(`Checkpoint calls · ${checkpointMix}`, `检查点调用 · ${checkpointMix}`))}</span></div><div><strong>${esc(jev.model || "—")}</strong><span>${esc(questionSet)} · ${esc(jev.authority || "—")}</span></div><div><strong>${esc(latestTokenSummary)}</strong><span>${esc(localized(`Latest tokens · ${tokenSourceLabel(latestTokens.source)} · ${latestIo} · ${duration(Number(latestCall.elapsed_ms || 0))}`, `本次 Token · ${tokenSourceLabel(latestTokens.source)} · ${latestIo} · ${duration(Number(latestCall.elapsed_ms || 0))}`))}</span></div><div><strong>${esc(totalTokenSummary)}</strong><span>${esc(localized(`Session tokens · ${tokenSourceLabel(totalTokens.source)} · ${totalIo} · avg ${duration(Number(callTotals.avg_elapsed_ms || 0))}`, `进程累计 Token · ${tokenSourceLabel(totalTokens.source)} · ${totalIo} · 平均 ${duration(Number(callTotals.avg_elapsed_ms || 0))}`))}</span></div><div><strong>${esc(route)}</strong><span>${esc(localized("Deterministic → Jev next action", "Deterministic → Jev 下一动作"))}</span></div><div><strong>${esc(comparison)}</strong><span>${num(jev.comparison?.shared_signals || 0)} ${esc(localized("shared signals", "共享信号"))}</span></div><div><strong>${esc(guidance)}</strong><span>${esc(localized("Applied advisory guidance", "已应用的 advisory 建议"))}</span></div></div>` : `<div class="empty">${esc(localized("Unknown — no Jev outcome has been observed for this project in the current process yet.", "未知 — 当前进程尚未观测到这个项目的 Jev 调用结果。"))}</div>`}</section>` : "";
  let bottleneck = "";
