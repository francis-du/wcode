...
impl TaskMonitor {
    ...
    pub(super) fn begin_intelligence_refresh(&self, workspace: &str) -> bool {
        let mut state = self.state.lock().expect("task monitor lock poisoned");
        ...
        stats.refresh_error = None;
        true
    }

    pub(super) fn finish_intelligence_refresh(&self, workspace: &str, error: Option<String>) {
        let mut state = self.state.lock().expect("task monitor lock poisoned");
        let stats = state.intelligence.entry(workspace.to_owned()).or_default();
    ...
    pub fn mark_concurrent_health(&self, healthy: bool) {
        let mut state = self.state.lock().expect("task monitor lock poisoned");
        state.public_url_healthy = Some(healthy);
        state.public_url_error = (!healthy).then(|| {
            "No currently verified public endpoint; providers recover independently".to_owned()
        });
        state.public_url_consecutive_failures = if healthy {
            0
        } else {
            state
                .tunnel_runtime
                .values()
                .map(|runtime| runtime.consecutive_failures)
                .max()
                .unwrap_or(0)
        };
    }
    ...
    pub fn register_tunnel(&self, provider: &str, public_url: &str) {
        ...
        let retry = state.tunnel_runtime.remove(&provider_runtime_key(provider));
        let runtime = state
            .tunnel_runtime
            .entry(endpoint_runtime_key(public_url))
            .or_insert_with(|| TunnelRuntimeState {
                ...
                death_count: retry.as_ref().map_or(0, |runtime| runtime.death_count),
                circuit_open: retry.as_ref().is_some_and(|runtime| runtime.circuit_open),
                retry_at: None,
                connected_at: None,
            });
        runtime.provider = provider.to_owned();
        runtime.url = Some(public_url.to_owned());
        runtime.role = "active".to_owned();
        ...
        state.public_url_last_checked = Some(now);
    }

    pub fn mark_tunnel_endpoint_probe(
        &self,
        public_url: &str,
        success: bool,
    ...
    pub fn mark_tunnel_retry(
        ...
        let mut state = self.state.lock().expect("task monitor lock poisoned");
        // Provider process recovery and endpoint health are separate state.
        // A stable endpoint can remain verified while its provider worker is
        // reconnecting, and one provider can own multiple verified URLs.
        let runtime = state
            .tunnel_runtime
            .entry(provider_runtime_key(provider))
            .or_insert_with(|| TunnelRuntimeState {
                provider: provider.to_owned(),
                url: None,
    ...
    pub fn mark_tunnel_stable(&self, provider: &str) {
        let mut state = self.state.lock().expect("task monitor lock poisoned");
        state.tunnel_runtime.remove(&provider_runtime_key(provider));
    ...
    pub fn remove_tunnel(&self, public_url: &str) {
        ...
        state
            .tunnel_runtime
            .remove(&endpoint_runtime_key(public_url));
...
fn endpoint_runtime_key(public_url: &str) -> String {
    format!("endpoint:{public_url}")
}

fn provider_runtime_key(provider: &str) -> String {
...
fn tunnel_runtime_statuses(state: &MonitorState, now: Instant) -> Vec<MonitorTunnelRuntimeStatus> {
    state
        ...
        .map(|runtime| MonitorTunnelRuntimeStatus {
