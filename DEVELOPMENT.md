...
- `src/power.rs` — process-lifetime idle-sleep inhibition for macOS, Linux, and Windows.
- `src/runtime_control.rs` — authenticated per-user `stop`/`restart` control and runtime ownership state.
