...
assert.deepEqual(ui.mcpActivity({ok:true,mcp_initialized:false}),{state:'waiting',age:null});
assert.deepEqual(ui.mcpActivity({ok:true,mcp_initialized:true}),{state:'initialized',age:null});
assert.deepEqual(ui.mcpActivity({ok:true,mcp_initialized:true,mcp_last_seen_seconds_ago:7.9}),{state:'active',age:7});
