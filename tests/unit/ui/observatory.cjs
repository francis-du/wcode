...
async function run(){
  ...
  const styles=['theme','shell','features','data','architecture','structure','responsive'].map(file=>fs.readFileSync(path.join(root,'src/ui/intelligence_web/styles',file+'.css'),'utf8')).join('\n');
  ...
  const visible=new Set(['stats','statusSummary','attention','architectureMetrics','componentCards','componentInspector','activity','resourceStatus','proofSummary','componentCount','projectIdentity','precisionBadge','lastUpdated']);
